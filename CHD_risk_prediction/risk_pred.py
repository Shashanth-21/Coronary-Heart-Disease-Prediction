...
# 6.FeatureSelection
# 7.SMOTE for balancing data
# 8.Scaling and Splitting Data
...
sns.barplot(x=missing_data.index, y=missing_data['Percentage'], data=missing_data, color='green')
...
sns.countplot(x='TenYearCHD', data=data_CHD, color='red')
...
# define pipeline
over = SMOTE(sampling_strategy=0.975, random_state=42)
under = RandomUnderSampler(sampling_strategy=0.975, random_state=42)
...
ann_y_train = ann.predict(X_train)
ann_y_pred = ann.predict(X_test)
ann_y_pred = ann_y_pred > 0.4
ann_y_train = ann_y_train > 0.4
print("The training set accuracy for ANN is {}".format(round(accuracy_score(y_train, ann_y_train)*100, 2)))
...
tree_predict_train = tree_clf.predict(X_train)
# accuracy
tree_accuracy = accuracy_score(y_test, tree_predict)
tree_train_accuracy = accuracy_score(y_train, tree_predict_train)
tree_acc = round(tree_accuracy * 100, 2)
tree_train_acc = round(tree_train_accuracy * 100, 2)
print("The training accuracy for dtree is {}".format(tree_train_acc))
...
knn_clf = GridSearchCV(KNeighborsClassifier(), params, cv=10, n_jobs=-1)
...
knn_train_acc = round(knn_train_accuracy * 100, 2)
knn_f1 = f1_score(y_test, knn_predict)
print("Using KNN we get a train accuracy of {}".format(knn_train_acc))
...
model = XGBClassifier(max_depth=12, subsample=1,
                      n_estimators=1750, learning_rate=.00900,
                      min_child_weight=1, random_state=42)
model.fit(X_train, y_train)
y_preds = model.predict(X_test)
y_train_preds = model.predict(X_train)
xgb_accuracy = accuracy_score(y_test, y_preds)
xgb_train_accuracy = accuracy_score(y_train, y_train_preds)
xgb_f1 = f1_score(y_test, y_preds)
xgb_acc = round(accuracy_score(y_test, y_preds)*100, 2)
xgb_train_acc = round(xgb_train_accuracy * 100, 2)
print("Train accuracy for XGB is {}".format(xgb_train_acc))
